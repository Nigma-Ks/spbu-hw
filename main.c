...
int main() {
    ...
    if (choiceOfCalculatingMethod == 1) {
        printf("%d member in Fibonacci sequence is %lu!", numberInSequence, nFibonacciMemberRecursive(numberInSequence));
    }else{
        printf("%d member in Fibonacci sequence is %lu!", numberInSequence, nFibonacciMemberIterative(numberInSequence));
    }
    return 0;
}
